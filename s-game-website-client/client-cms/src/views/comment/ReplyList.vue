<template>
    ...
    <div class="tableSample">
        <!-- 头 -->
        <div class="header">
            <div class="header-left"><p class="title">回复列表</p></div>
            <div class="header-right">
                <div class="comment-id">
                    <el-tooltip class="item" effect="dark" content="仅显示指定评论ID的回复" placement="top">
                        <Span>按评论ID搜索</Span>
                    </el-tooltip>
                    <el-input size="medium" type="number" v-model="searchCommentId"></el-input>
                </div>
                        ...
                        <div class="summary">
                            <el-form label-position="left" inline class="demo-table-expand">
                                ...
                                <el-form-item label="评论ID">
                                    <span>{{ props.row.parent_reply_id }}</span>
                                </el-form-item>
                                <el-form-item label="评论昵称">
                                    <span>{{ props.row.reply_user_name }}</span>
                                </el-form-item>
                                ...
                                <el-form-item label="用户头像">
                                    <span>{{ props.row.head_icon_id }}</span>
                                </el-form-item>
                                <el-form-item label="屏蔽发言">
                                    <span>{{ props.row.cover_text }}</span>
                                </el-form-item>
                                ...
                                <el-form-item label="点踩">
                                    <span>{{ props.row.hatecount }}</span>
                                </el-form-item>
                                <el-form-item label="举报">
                                    <span>{{ props.row.reportcount }}</span>
                                </el-form-item>
                ...
                <el-table-column label="操作" fixed="right" width="200">
                    <template slot-scope="scope">
                        <el-button
                                ...
                                v-auth="{ auth: '更新回复', type: 'disabled'}">编辑
                        </el-button>
                        <el-button v-if="scope.row.delete_at"
                                   ...
                                   v-auth="{ auth: '恢复回复', type: 'disabled'}">恢复
                        </el-button>
                        <el-button
                                type="danger"
                                size="mini"
                                ...
                                v-auth="{ auth: '删除回复', type: 'disabled'}">删除
                        </el-button>
                    </template>
                </el-table-column>
            </el-table>
            ...
            <div>
                <div class="dialog-container" v-if="dialogFormVisible" @click.self="handleClose">
                    <div class="dialog-box">
                        <reply-editor id="reply-editor" ref="replyEditor"
                                        v-if="editorReplyDate"
                                        :replyDate="editorReplyDate"
    ...
    export default {
        ...
        data() {
            return {
                ...
                editorReplyDate: null,
                editorReplyIndex: 0,
                tableColumn: [
                    {prop: 'id', label: 'ID', width: 60, noRepeat: true,},
                    {prop: 'parent_comment_id', label: '评论ID', width: 50},
                    {prop: 'reply_user_name', label: '评论昵称', width: 50},
                    {prop: 'nickname', label: '用户昵称', width: 100},
                    {prop: 'email', label: '用户邮箱', width: 100},
                    {prop: 'head_icon_id', label: '用户头像', width: 50},
                    {prop: 'cover_text', label: '屏蔽发言', width: 50},
                    {prop: 'content', label: '文本', width: 200},
                    {prop: 'likecount', label: '点赞', width: 50},
                    {prop: 'hatecount', label: '点踩', width: 50},
                    {prop: 'reportcount', label: '举报', width: 50},
        ...
        methods: {
            ...
            async _getTableData() {
                try {
                    this.loading = true
                    const res = await comment.getReplys({
                        parent_comment_id: this.searchCommentId,
            ...
            handleEdit(index, row) {
                if (!row) return;
                this.editorReplyIndex = index;
                this.editorReplyDate = {...row};
            ...
            async handleDelete(row) {
                this.$confirm('此操作将删除该信息, 是否继续?', '提示', {
                    ...
                    this.loading = true;
                    const res = await comment.deleteReply(row.id);
                    if (res.error_code === 0) {
                        this.dialogFormVisible = false;
                        this.editorReplyDate = null;
                        this.editorReplyIndex = 0;
            ...
            async handleRecover(row) {
                this.$confirm('此操作将恢复并在网页上显示该信息, 是否继续?', '提示', {
                    ...
                    this.loading = true;
                    let id = row.id;
                    const res = await comment.recoverReply({
                        id,
                    });
                    if (res.error_code === 0) {
                        this.$message.success(`${res.msg}`)
            ...
            async submitReplyDate() {
                try {
                    this.loading = true;
                    const res = await comment.editReply(this.editorReplyDate.id, this.editorReplyDate)
                    if (res.error_code === 0) {
                        this.dialogFormVisible = false
                        this.editorReplyDate = null;
                        this.editorReplyIndex = 0;
                    ...
                    this.editorReplyIndex = 0;
                    this.loading = false;
                    this.$message.error('参数错误，回复修改上传失败')
                }
            },
            // 重置表单
            resetReplyDate() {
                let row = this.tableData[this.editorReplyIndex];
                this.editorReplyDate = {...row};
